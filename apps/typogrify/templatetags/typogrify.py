...
def widont(text):
    ...
    >>> widont('<ul> <li> one </li> <li> two three </li></ul>')
    '<ul> <li> one </li> <li> two&nbsp;three </li></ul>'
    """
    widont_finder = re.compile(r"""([^<>\s])                            # must be preceded by non-tag non-space characters
                                   ...
                                   """, re.VERBOSE)
    return mark_safe(widont_finder.sub(r'\1&nbsp;\2', text))
