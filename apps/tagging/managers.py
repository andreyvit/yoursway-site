...
class TagManager(models.Manager):
    ...
    def _get_usage(self, model, counts=False, min_count=None, extra_joins=None, extra_criteria=None, params=None):
        ...
        query = """
        SELECT DISTINCT %(tag)s.id, %(tag)s.name, %(tag)s.translation%(count_sql)s
        FROM
            %(tag)s
        ...
        WHERE %(tagged_item)s.content_type_id = %(content_type_id)s
            %%s
        GROUP BY %(tag)s.id, %(tag)s.name, %(tag)s.translation
        %%s
        ...
        for row in cursor.fetchall():
            t = self.model(id=row[0], name=row[1], translation=row[2])
            if counts:
                t.count = row[3]
            tags.append(t)
        return tags
    ...
    def related_for_model(self, tags, model, counts=False, min_count=None):
        ...
        query = """
        SELECT %(tag)s.id, %(tag)s.name, %(tag)s.translation%(count_sql)s
        ...
        related = []
        for row in cursor.fetchall():
            tag = self.model(id=row[0], name=row[1], translation=row[2])
            if counts is True:
                tag.count = row[3]
            related.append(tag)
        return related
